...
const photobox = {
     ...
     * @param {string} projectId - 项目ID（001或010等）
     */
    createImageSlider(container, images, labelEl, indexEl, projectId = '001') {
        // 创建滑动容器
        ...
        const expandedIndex = document.createElement('span');
        expandedIndex.className = 'card-index';
        expandedIndex.textContent = `1 / ${images.length}`;
        ...
        const updateSlider = () => {
            ...
            // 更新索引显示
            expandedIndex.textContent = `${currentIndex + 1} / ${images.length}`;
        };
     ...
     * 创建媒体滑动器（支持图片和视频混合）
     * @param {HTMLElement} container - 容器元素
     * @param {Array} mediaList - 媒体对象数组，每个对象包含 type 和 src
     * @param {HTMLElement} labelEl - 标签元素
     * @param {HTMLElement} indexEl - 索引元素
     * @param {string} projectId - 项目ID
     */
    createMediaSlider(container, mediaList, labelEl, indexEl, projectId = '010') {
        // 创建滑动容器
        const slider = document.createElement('div');
        slider.className = 'image-slider';
        ...
        // 添加所有媒体（独立容器）
        const slides = [];
        let currentVideo = null; // 跟踪当前播放的视频
        ...
        mediaList.forEach((media, i) => {
            ...
            if (media.type === 'video') {
                ...
                playBtn.style.border = '2px solid rgba(0, 240, 255, 0.8)';
                playBtn.style.color = '#00f0ff';
        ...
        const expandedIndex = document.createElement('span');
        expandedIndex.className = 'card-index';
        expandedIndex.textContent = `1 / ${mediaList.length}`;
        container.appendChild(expandedIndex);
        ...
        const updateSlider = () => {
            ...
            // 更新索引显示
            expandedIndex.textContent = `${currentIndex + 1} / ${mediaList.length}`;
        };
